...
<!--####################################################################
	...
	@page guides/contributing/api-design-guidelines
######################################################################## -->
<html lang="en">
<head>
	<meta charset="utf-8">
	<title>CanJS - API Design Guidelines</title>
...
<div class="breadcrumb">
	...
	<li><a href="api-design-guidelines.html">API Design Guidelines</a></li>
  ...
  <section class="title">
	<div class="page-type">
		<h1>API Design Guidelines</h1>
			<ul class="title-social">
				...
				<li>
					<a class="button-link" href="//github.com/canjs/canjs/edit/master/docs/can-guides/contribute/api-design-guidelines.md">Edit on GitHub</a>
				</li>
    ...
    <p>Learn how to design APIs which are consistent with CanJS.</p>
...
<section class="on-this-page-table">
    ...
    <section class="body">
    <h2>Overview</h2>
<p>CanJS has many goals, one of which is providing a consistent interface and experience for developers using similar functionality or usage patterns across projects. Ideally, a developer should have a rough understanding of how to use something new based on what they have already used in CanJS.</p>
<p>This document is geared towards contributors who are designing new APIs for CanJS and developers of libraries meant to work alongside CanJS projects. Below are useful conventions for designing APIs, which have been adopted by CanJS projects or will be aligned with in future releases.</p>
<h2>Conventions</h2>
<h3>Variable casing</h3>
<p>All methods and properties should be camelCase. Class and constructor function names are exclusively PascalCase.</p>
<h3>Methods are verbs, properties are nouns</h3>
<p>A developer should be able to tell by looking at an identifier whether it is a function that can be called or an object/property that has a value. In JavaScript, functions can be values so be careful to make the distinction between a <em>method</em> (verb) and a function being passed around as data (noun). For example, <code>handleEvent</code> is a method, but if that method were being passed around <code>eventHandler</code> is more appropriate.</p>
<p>Methods which are only verbs such as <code>jump()</code> are discouraged when it is not clear what should be passed to it. A better name <code>jumpOverPerson()</code> provides a hint as to what the method expects.</p>
<h3>Private methods and variables</h3>
<p>Any method or variable beginning with one or more underscores (<code>_</code>) is private to the project and should not be used externally. Ideally these cases are minimized.</p>
<p>Note: there is a temporary convention for Symbols which is an exception to the above. <strong>Documented</strong> variables with two leading underscores, for example <code>__keys</code>, are safe to use as they are intentionally exposed for environments where symbols are not supported.</p>
<h3>Private modules</h3>
<p>Any module file which is prefixed with one or more dashes (<code>-</code>) is private to the project and should not be used externally. Ideally these cases are minimized.</p>
<h3>Creating new things</h3>
<p>In CanJS, new &quot;things&quot; are created by either calling a constructor or class with the <code>new</code> keyword or calling functions which begin with <code>make</code>. For example, to create a Promise from an existing Promise-like object, <code>can-util</code> provides <code>makePromise</code>. Use of <code>make</code> is more explicit than, for example, <code>getPromise</code> because the underlying implementation is <em>creating a value</em>, not retrieving a value.</p>
<h3>Registries</h3>
<p>Throughout CanJS we use a registry pattern. A registry, in abstract terms, is an object where <em>named</em> items are added, retrieved, and removed dynamically over the course of a program's life. For example, a component registry is where named components are added, retrieved, and removed. Registries can be implemented with the following interface:</p>
<pre><code class="language-js">interface Registry&lt;RegistryKey, RegistryItem&gt; {
...
<script type="text/javascript">
  window.docObject = {"src":{"path":"docs/can-guides/contribute/api-design-guidelines.md"},"description":"Learn how to design APIs which are consistent with CanJS. \n","name":"guides/contributing/api-design-guidelines","title":"API Design Guidelines","type":"page","parent":"guides/contribute","comment":" ","pathToRoot":"../../.."};
</script>
</article>
