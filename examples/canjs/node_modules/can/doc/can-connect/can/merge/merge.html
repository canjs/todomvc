...
<head>
	<meta charset="utf-8">
	<title>CanJS - can-connect/can/merge/merge</title>
...
<div class="breadcrumb">
  ...
  <section class="title">
	<div class="page-type">
		<h1>can-connect/can/merge/merge</h1>
    ...
    <p>Update nested data structures correctly with the response from the server.</p>
...
<div class="signature">
  ...
  <p>Overwrites <a href="../map/map.html" title="Make a connection use a map type.">can-connect/can/map/map</a>'s instance callbacks
so they use <a href="../../helpers/map-deep-merge.html" title="Perform a smart merge of deeply nested maps and lists.">can-connect/helpers/map-deep-merge</a>.  <a href="../../helpers/map-deep-merge.html" title="Perform a smart merge of deeply nested maps and lists.">can-connect/helpers/map-deep-merge</a>
is able to make minimal changes to instances and lists given raw data.  Use this behavior
after the <a href="../map/map.html" title="Make a connection use a map type.">can-connect/can/map/map</a> behavior:</p>
...
<p>The connection's <a href="../map/map._Map.html" title="Specify what type of Map should be hydrated.">Map</a>, <a href="../map/map._List.html" title="Specify what type of CanList should be hydrated.">List</a>
and any other types that they reference must be property configured.  That configuration
is discussed in the <code>Use</code> section below.</p>
    ...
    <section class="body">
    <h2>Use</h2>
<p>To use <code>can-connect/can/merge/merge</code>, you have to:</p>
<ol>
<li>Add the behavior after <a href="../map/map.html" title="Make a connection use a map type.">can-connect/can/map/map</a>, and</li>
<li>Make sure all types, especially <code>List</code> types are properly configured.</li>
</ol>
<p>Adding the <code>can-connect/can/merge/merge</code> behavior after <a href="../map/map.html" title="Make a connection use a map type.">can-connect/can/map/map</a>
is pretty straightforward.  When you create a custom connection, create it as follows:</p>
<pre><code class="language-js">var canMergeBehavior = require(&quot;can-connect/can/merge/merge&quot;);
var canMapBehavior = require(&quot;can-connect/can/map/map&quot;);
...
<p>For <a href="../../helpers/map-deep-merge.html" title="Perform a smart merge of deeply nested maps and lists.">can-connect/helpers/map-deep-merge</a> to
merge correctly, it needs to know how to uniquely identify an instance and
be able to convert raw data to instances and lists. <code>map-deep-merge</code> looks for
this configuration on the <code>.algebra</code> and <code>.connection</code> properties of the
...
<p>This is more easily understood if the <code>ClassRoom</code> has a <code>students</code> property that
is a list of <code>Student</code> instances like:</p>
<pre><code class="language-js">var ClassRoom = DefineMap.extend({
    students: Student.List
});
</code></pre>
<p>To be able to uniquely identify <code>Student</code> instances, make sure <code>Student</code>
has an <code>algebra</code> property that is configured with the unique identifier property:</p>
<pre><code class="language-js">Student = DefineMap.extend({ ... });

Student.algebra = new set.Algebra(set.props.id(&quot;_id&quot;))
</code></pre>
<p>Also, make sure that <code>Student.List</code> points its <a href="../../../can-define/list/list.prototype.wildcardItems.html" title="Define default behavior for items in the list.">#</a>
definition to <code>Student</code> like the following:</p>
<pre><code class="language-js">Student.List = DefineList.extend({
    &quot;#&quot;: Student
});
</code></pre>
<p>Finally, the default method used to create a <code>Student</code> will be <code>new Student(props)</code>.  However,
if <code>Student</code>'s have a <code>.connection</code>, the <code>.connection.hydrateInstance(props)</code> will be
used.  This is useful if <code>Student</code>s should be looked up in their <a href="../../constructor/store/store.instanceStore.html" title="A store of instances mapped by base.id.">instanceStore</a>.</p>
<p>For example, <code>Student</code> might have a connection like:</p>
