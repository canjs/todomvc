...
<head>
	<meta charset="utf-8">
	<title>CanJS - can-connect/can/constructor-hydrate/constructor-hydrate</title>
...
<div class="breadcrumb">
  ...
  <section class="title">
	<div class="page-type">
		<h1>can-connect/can/constructor-hydrate/constructor-hydrate</h1>
    ...
    <p>Always check the <a href="../../constructor/store/store.instanceStore.html" title="A store of instances mapped by base.id.">instanceStore</a> when creating new instances of the connected <a href="../map/map._Map.html" title="Specify what type of Map should be hydrated.">Map</a> type.</p>
...
<div class="signature">
  ...
  <pre><code> Overrides &lt;a href=&quot;../../../can-define/map/map.html&quot; title=&quot;Create observable objects.&quot;&gt;can-define/map/map&lt;/a&gt;'s `setup` method and checks whether a newly created instance already
 exists in &lt;a href=&quot;../../constructor/store/store.instanceStore.html&quot; title=&quot;A store of instances mapped by base.id.&quot;&gt;instanceStore&lt;/a&gt;. If it exists that instance will be
 returned instead of a new object.

 This behavior has to be used with [can-connect/constructor/store] and &lt;a href=&quot;../map/map.html&quot; title=&quot;Make a connection use a map type.&quot;&gt;can-connect/can/map/map&lt;/a&gt; behaviors.
</code></pre>
    ...
    <section class="body">
    <h2>Use</h2>
<p>This behavior is useful if <code>Type</code> converters of <a href="../../../can-define/map/map.html" title="Create observable objects.">can-define/map/map</a> are used in multiple places of your app.
In which case if a property is set with an id of an already created instance then the connection behavior will
check <a href="../../constructor/store/store.instanceStore.html" title="A store of instances mapped by base.id.">instanceStore</a>. If there is already an instance with the same id
then it will be returned instead of a new object.</p>
<p>Let's say we have the following page state with two properties which are of type <code>Student</code>:</p>
<pre><code class="language-js">var myPage = new (DefineMap.extend({
    student: { Type: Student },
       teamLead: { Type: Student },
}));
</code></pre>
<p>The type <code>Student</code> is a DefineMap with <code>can-connect</code> capabilities:</p>
<pre><code class="language-js">var Student = DefineMap.extend({});
...
<p>Now lets say your application loads <code>student</code> in a regular way using <code>Student.get()</code>, and it gets data
for <code>teamLead</code> from somewhere else. Also let's the team lead is the same person as student:</p>
<pre><code class="language-js">myPage.student = Student.get({id: 1});

myPage.loadTeamLead().then( function(person){ myPage.teamLead = person; } );
</code></pre>
<p>Without <a href="constructor-hydrate.html" title="Always check the store.instanceStore when creating new instances of the connected map._Map type.">can-connect/can/constructor-hydrate/constructor-hydrate</a> we would end up with two instances of <code>Student</code> with the same id.
Also, the <code>teamLead</code> would not be referencing an instance that is stored in connection's <code>instanceStore</code>
and thus will loose real-time updates if <a href="../../real-time/real-time.html" title="Update lists to include or exclude instances based
on set logic.">can-connect/real-time/real-time</a> was used.</p>
<p>This behavior solves this problem by checking <code>instanceStore</code> before creating a new instance. So, in our app
it will return the existing instance and give it to <code>teamLead</code>. Now both <code>myPage.student</code> and <code>myPage.teamLead</code>
are referencing the same instance:</p>
